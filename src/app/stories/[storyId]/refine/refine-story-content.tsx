...
export function RefineStoryContent() {
  ...
  const updateStoryScript = useMutation(api.guidedStory.updateStoryScriptPublic);
  const generateSegments = useMutation(api.guidedStory.generateSegmentsMutation);
  ...
  const { length: estimatedVideoLength, wordCount, characterCount } = estimateVideoLength(story?.script || "");
          ...
          {openDialog === DialogType.GenerateSegments && (
            <>
              <DialogHeader>
                ...
                <DialogDescription className="text-gray-900 space-y-2">
                  <span className="block">
                    Vertical videos are ideal for platforms like TikTok and Instagram Reels.
                  </span>
                  <span className="block">
                    Horizontal videos are better suited for YouTube and traditional video players.
                  </span>
                  <span className="block font-bold">
                    Note: Once set, the orientation cannot be changed without regenerating all images, so choose carefully!
                  </span>
                </DialogDescription>
              </DialogHeader>
              <div className="flex justify-between space-x-4 mt-4">
              ...
              <DialogFooter className="mt-8">
                ...
                >
                  {isGenerating ? "Generating..." : `Generate (${estimatedCredits} credits)`}
                </Button>
              </DialogFooter>
            </>
          )}
